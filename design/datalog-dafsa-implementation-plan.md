...
- **Deferred (future):** pointwise/stratified negation over recursive predicates — **VERDICT (2026-08-18): NOT tractable in the least-fixpoint semi-naive engine** (requires well-founded/alternating-fixpoint semantics — a second evaluation driver incompatible with magic-sets/topdown permutation-invariance and IVM/DRed delta propagation); supported subset = **stratified negation of recursive predicates from strictly-higher strata** (works, fixture-locked in `tests/test_m2.c`) — and the IVM/DRed fallback-gap follow-ups (see below).
