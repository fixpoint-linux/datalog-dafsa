...
**Status:** Implemented M0-M9 + v2 IVM and v2 extensions (2026-08-17): fact store, interner, parser/compiler/VM, aggregates/equality/disjunction, snapshot publish + mmap query path, regex walker, permutation indices + hash-join + automatic perm selection, durability (lock + interner + WAL), magic-sets/QSQ top-down, arithmetic/string builtins, IVM (Slices 0-5 incl. DRed deletion), bushy plans, variadic relations, nested terms/lists, range-index order-statistics (rank/select/range/count, iterator + merge-join, lazy OP_RANGE), time-travel / as-of snapshot queries. 300+ tests green. **Remaining deferred (future):** pointwise/stratified negation over recursive predicates — **VERDICT (2026-08-18): pointwise/locally-stratified negation over recursion is NOT tractable in this least-fixpoint semi-naive engine** (it requires well-founded/alternating-fixpoint semantics — a second evaluation driver incompatible with magic-sets/topdown permutation-invariance and IVM/DRed delta propagation); the supported subset is **stratified negation of recursive predicates from strictly-higher strata** (works, fixture-locked in `tests/test_m2.c`), and the IVM/DRed fallback-gap follow-ups (see implementation-plan §M8+). Trace/JIT REJECTED (see §10.8).
...
8. **Perf vs Souffle on raw rule eval** -- Souffle synthesizes native code per rule; we interpret bytecode. We'll be slower on the fixpoint itself. **Mitigation:** lean on the DAFSA density advantage for storage and the mmap zero-copy read path. **Watch:** if a workload is dominated by complex rules over small data, Souffle wins outright; the user's value prop is embedding + storage density, not raw rule throughput.
   ...
   **Reconsidered with DynASM (2026-08-15):** DynASM (LuaJIT's runtime assembler) does solve the asm→executable half QBE lacked — its macros emit raw machine code into an executable buffer at runtime, no separate assembler, no runtime toolchain. But the verdict stands: the bottleneck is the intrinsic C tuple/DAFSA data-access cost (a memory-latency-bound DAFSA DFS walk, `ts_*` hash/sort, and the ~40% post-fixpoint DAFSA bulk-build that runs once per stratum, outside any per-tuple trace), not bytecode dispatch. Full trace specialization (the only DynASM path that would move the needle) recovers only the call/materialization overhead — a ~3–15% ceiling on small in-cache relations, ~0–3% on large ones — and would require hand-writing x86-64 + aarch64 machine code that reimplements the vendored engine's private `State`/`Edge` layout, plus a Lua preprocessor build dependency (verified absent on the build host), plus a permanent byte-identical second code path. Unjustified; interpreter-level optimization recovers the same single-digit % at zero dependency cost.
...
11. **Deferred (v2, future):** **pointwise / stratified negation over recursive predicates** — **VERDICT (2026-08-18): pointwise/locally-stratified negation over RECURSION is NOT tractable in the least-fixpoint semi-naive engine** (requires well-founded/alternating-fixpoint semantics — a second evaluation driver incompatible with magic-sets/topdown permutation-invariance and IVM/DRed delta propagation); the supported subset is **stratified negation of recursive predicates from strictly-higher strata** (works, fixture-locked in `tests/test_m2.c`: `isolated(X):-node(X),!reach(X)` over a recursive `reach`). Negation reading an adorned-closure predicate is REJECTED as unsound in both magic-sets paths — see `design/datalog-dafsa-topdown-magic.md` STAGE C; and the **IVM/DRed fallback-gap follow-ups** (recursion+deletion via DRed-over-recursion, aggregate shapes under delete, negation-on-insert, `OP_WALK`/`OP_LOOKUP_PERM`/`OP_HASH_JOIN` override, bushy `OP_MAT_*` delta) — see implementation-plan §M8+. (IVM SHIPPED v2 Slices 0-5; magic-sets/QSQ top-down SHIPPED v2 as `dl_query_topdown`/`_adorn`, STAGE A+B+C verified; bushy plans, variable arity, nested terms/lists, time-travel SHIPPED v2; trace/JIT REJECTED — see §10.8.)
